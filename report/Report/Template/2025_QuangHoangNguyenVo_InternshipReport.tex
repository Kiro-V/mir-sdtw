...
\begin{table}[t]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        150/150 & Binary Cross-entropy & 78.5 & 45 \\
        \hline
        16/150 & Soft-DTW (uniform) & 74.3 & 200 \\
        75/150 & Soft-DTW (uniform) & 75.1 & 200 \\
        \hline
        16/150 & Soft-DTW (last) & 76.2 & 200 \\
        75/150 & Soft-DTW (last) & 77.0 & 200 \\
        \hline
...
\end{table}
Table \ref{table:results_strong} summarizes the results of the baseline model trained on strong target with binary cross-entropy loss and the model trained on soft target with soft-DTW loss. Here we choose two different soft lengths, 16 and 75 to evaluate the effect of the reduction ratio on the performance. We also compare two different padding strategies, uniform and last element repetition.
From the results, we can see that the model trained on strong target with binary cross-entropy loss achieves the best performance, with an F1-score of 78.5\%. This is expected, as the model has access to the most accurate and detailed information during training.
For the model trained on soft target with soft-DTW loss, we observe that the performance is slightly lower than the baseline, with an F1-score of 74.3\% for soft length 16 and 75.1\% for soft length 75 using uniform padding strategy. However, when using the last element repetition padding strategy, the performance improves to 76.2\% for soft length 16 and 77.0\% for soft length 75. This suggests that the choice of padding strategy can have a significant impact on the performance of the model.


\section{Comparison with CTC Loss}
\label{section:comparison_ctc
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Conclusions}
